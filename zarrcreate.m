...
%     ShardSize                - Defines the sharding layout specification
...
arguments
    ...
    options.ShardSize (1,:) double {mustBeFinite, mustBePositive} = []
...
if any(size(datasize) ~= size(options.ShardSize))
    error("MATLAB:zarrcreate:chunkDimsMismatch",...
...
if isscalar(datasize)
    ...
    if ~isempty(options.ShardSize)
        options.ShardSize = [1 options.ShardSize];
    end
end

zarrObj = Zarr(filepath);
zarrObj.create(options.Datatype, datasize, options.ChunkSize, options.ShardSize, options.FillValue, options.Compression)
